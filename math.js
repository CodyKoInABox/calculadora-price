const PriceMath = (() => {
  ...
  function clampCurveValue(value, step = 0.05) {
  ...
  function curveWeightAt(progress, curveControls) {
   ...
   * @returns {{ error: string } | { property, down, principal, months, totalInterest, totalRegular, totalBalloon, totalBalloonsPlanned, schedule }}
   */
  function simulateFinancing({ property, down, months, rate, mode, balloons, curveControls }) {
    ...
    // Base installment sequence. Binary search finds a scale factor so final balance → 0,
    // preserving either fixed Price installments or the requested growing pattern.
    let base = [];
    if (mode === 'price') {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      if (closingBalance(mid) > 0) low = mid; else high = mid;
    }
    const factor = high;
